...
### Fixed
- **Better Anti-Dupe was never detected.** The plugin looked for its old name, so claimed elytras weren't marked as the claimer's own.
...
- **Protection now stops** buckets, pistons, withers and falling blocks.
- **Folia:** `/betterend tp` and saving from the menu work.
- **`/betterend tp`** lands on top of the city's base tower instead of possibly over the void.
- **`/betterend delete`** clears the city's elytra claims at once and lets it be found again without a restart.
- **`/betterend resetloot`** no longer stalls the server on a name that never joined.
